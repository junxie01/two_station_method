...
c      include "/home/junxie/opt/sac/include/sacf.h"
...
c      implicit none
      ...
      character(80) names,name1,name2,output,out_put
      out_put="test.sac"
      cc=0
      amax=0
      summ1=0
      ...
      if(nargs.ne.1)then
          ...
          write(*,*)'para.dat:'
          write(*,*)'file1 file2 pmin pmax vmin vmax out phv_reference'
          call exit(-1)
      endif
      call getarg(1,para)
      open(99,file=para) ! read in the parameter data
  200    read(99,*,err=201,end=201)sacfile1,sacfile2,pmin,pmax,vb,ve,phvref
